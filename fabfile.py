...
try:
    from fabsettings import WF_HOST, PROJECT_NAME, REPOSITORY, USER, PASSWORD,PASSWORD2, VIRTUALENVS, SETTINGS_SUBDIR, MACHINE
except ImportError:
...
def install_app():
    ...
    upload_template('fabtemplates/gunicorn.conf',
...
def install_supervisor():
    ...
    upload_template('./fabtemplates/supervisord.conf',
    ...
    upload_template('./fabtemplates/start_supervisor.sh',
...
def reload_app(arg=None):
    ...
    if arg <> "quick":
        with cd(env.settings_dir):
            ...
            _ve_run(env.project, "python2.7 manage-prod.py  migrate")
            _ve_run(env.project, "python2.7 manage-prod.py  collectstatic")

    restart_app()
    
def refresh_app(arg=None):
    """Pulls app and refreshes requirements"""
...
def _webfaction_create_app(app):
    ...
    session_id, account = server.login(USER, PASSWORD2, MACHINE)
    try:
